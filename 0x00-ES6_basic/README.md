...
0. Modify function taskFirst to instantiate variables using `const` & function taskNext to instantiate variables using `let`
...
bob@dylan:~$ 
bob@dylan:~$ npm run dev 0-main.js (or npx babel-node 0-main.js)
...
3. Condense the internals of the following function to 1 line - without changing the name of each function/variable. The key here to define default parameter values for the function parameters.
...
7. Notice how the keys and the variable names are the same? Modify the following function’s budget object to simply use the keyname instead.
...
13. Write a function named createIteratorObject, that will take into argument a report Object created with the previous function createReportObject. This function will return an iterator to go through every employee in every department.
